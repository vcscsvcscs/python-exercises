...
import unittest
from exercise_1_weathermonitoringtool import query_weather_data, temperature_to_celsius_and_fahrenheit, time_until_sunrise_or_sundown
...
class TestWeatherMonitoringTool(unittest.TestCase):
    ...
    def test_time_until_sunrise_or_sundown(self):
        ...
        api_key = "76717af25a09a9c55ca00b44f771acc0"
        weather_data = query_weather_data(city, api_key)
        untilwhat,_ = time_until_sunrise_or_sundown(weather_data['sys']['sunrise'], weather_data['sys']['sunset'])
        if(datetime.datetime.now().timestamp() < weather_data['sys']['sunrise']):
